...
func TestBodyExcludesParamFields(t *testing.T) {
	...
	assert.Equal(t, "#/components/schemas/patchReq", schema["$ref"], "body schema")

	body := schemas(t, doc)["patchReq"].(map[string]any)
...
func TestBodySchemaJSONShape(t *testing.T) {
	...
	doc := serveDoc(t, d, r)
	raw, err := json.Marshal(schemas(t, doc)["patchReq"])
